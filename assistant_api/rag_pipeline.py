...
class RAGPipeline:
    ...
    def __init__(self, 
                 ...
                 data_file: str = "data/rag_architecture.md",
                 model: str = None,
        ...
        Args:
            ...
            persist_directory: директория ChromaDB
        """
        # Проверка API ключа
        ...
        if self.vector_store.collection.count() == 0:
            if not os.path.exists(data_file):
                raise FileNotFoundError(f"Файл с документами не найден: {data_file}")
...
Ответ:"""
    ...
    @staticmethod
    def _normalize_context_docs(context) -> List[Dict[str, Any]]:
        """Приводит context из кеша (строки) к единому формату dict."""
        if not context:
            return []
        if isinstance(context[0], str):
            return [{"text": text} for text in context]
        return context
        ...
        if use_cache:
            ...
            if cached_result:
                ...
                return {
                    ...
                    "from_cache": True,
                    "context_docs": self._normalize_context_docs(cached_result.get("context")),
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Тестовые запросы
        test_queries = [
            "Что такое hybrid retrieval?",
            "Какие компоненты ingestion pipeline?",
            "Почему vector-only RAG недостаточен?",
        ]
