...
class VectorStore:
    ...
    def _preprocess_markdown(self, text: str) -> str:
        """Нормализация Markdown перед chunking."""
        # Удаление YAML frontmatter
        text = re.sub(r'^---\n.*?\n---\n', '', text, count=1, flags=re.DOTALL)
        # Горизонтальные разделители и заголовки как границы абзацев
        text = re.sub(r'\n---+\n', '\n\n', text)
        text = re.sub(r'\n(#{1,6}\s)', r'\n\n\1', text)
        return text.strip()
    ...
    def _chunk_text(self, text: str, chunk_size: int = 600, overlap: int = 120) -> List[str]:
        ...
        Стратегия:
        1. Нормализация Markdown (frontmatter, ---, заголовки)
        2. Приоритет абзацам (разделение по \n\n)
        3. Разбиение длинных абзацев по предложениям
        4. Сохранение контекста через overlap
        5. Учёт минимального и максимального размера чанка
        ...
        # Пост-обработка: фильтруем слишком короткие чанки (заголовки MD могут быть короче)
...
if __name__ == "__main__":
    ...
    # Загрузка документов
    if os.path.exists("data/rag_architecture.md"):
        vector_store.load_documents("data/rag_architecture.md")
